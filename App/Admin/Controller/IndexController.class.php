...
class IndexController extends CommonController {
	...
	public function code(){
        ...
        if (I('get.code_len')) {
            $verify->length = intval(I('get.code_len'));
        }
        if ($verify->length > 8 || $verify->length < 2) {
            $verify->length = 4;
        }
        if (I('get.font_size')) {
            $verify->fontSize = intval(I('get.font_size'));
        }
        if (I('get.width')) {
            $verify->imageW = intval(I('get.width'));
        }
        if ($verify->imageW <= 0) {
            $verify->imageW = 130;
        }
        if (I('get.height')) {
            $verify->imageH = intval(I('get.height'));
        }
        if ($verify->imageH <= 0) {
            $verify->imageH = 50;
        }
