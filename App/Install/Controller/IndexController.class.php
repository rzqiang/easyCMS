...
class IndexController extends Controller {
	...
	public function check($item = ''){
		switch ($item){
			...
			case 'mysql':
				//'MYSQL 扩展'
				extension_loaded('mysqli') && extension_loaded('pdo_mysql')? $this->success('通过') : $this->error('未开启mysqli或者mysql_pdo扩展');
				break;
			case 'gd':
				//'GD 扩展'
	...
	public function dbTest($dbhost = '', $dbport = '3306', $dbname = '', $tablepre = '', $dbuser = '', $dbpw = ''){
		...
		$dbhost = $dbhosts[0];
        $lnk = @mysqli_connect($dbhost.':'.$dbport, $dbuser, $dbpw) or die('2');
		
		$server_info = mysqli_get_server_info($lnk);
		if($server_info < '4.0') {
		    exit('6');
        }
		
		if(!mysqli_select_db($dbname)) {
			if(!@mysqli_query($lnk,"CREATE DATABASE `{$dbname}` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci")) {
	...
	function install($dbhost = '', $dbport = '3306', $dbname = '', $tablepre = '', $dbuser = '', $dbpw = '', $username = '', $password = '', $email = ''){
		$dbhosts = explode(',', $dbhost);
		$dbhost = $dbhosts[0];
		$lnk = @mysqli_connect($dbhost.':'.$dbport, $dbuser, $dbpw) or die('Not connected : ' . mysqli_error());
		
		$version = mysqli_get_server_info($lnk);
		if($version > '4.0') {
			mysqli_query($lnk,"SET sql_mode=''");
		}
												
		if(!@mysqli_select_db($dbname)){
			@mysqli_query($lnk,"CREATE DATABASE `{$dbname}` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci");
			if(@mysqli_error()) {
				exit('1');
			} else {
				mysqli_select_db($dbname);
		...
		if(file_exists($dbfile)) {
			...
			$this->_sql_execute($lnk,$sql);
			//创建网站创始人
			$password_arr = password($password);
			$password = $password_arr['password'];
			$encrypt = $password_arr['encrypt'];
			$email = trim($email);
			$this->_sql_execute($lnk,"update ".$tablepre."admin set `username`='{$username}',`password`='{$password}',`roleid`='1',`encrypt`='{$encrypt}',`email`='{$email}' where `userid`='1'");
